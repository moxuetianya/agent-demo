...
def scan_unclaimed_tasks() -> list:
    ...
    if unclaimed:
        main_logger.debug(f"Found {len(unclaimed)} unclaimed tasks")
    return unclaimed
...
def claim_task(task_id: int, owner: str) -> str:
    with _claim_lock:
        ...
        if not path.exists():
            main_logger.warning(f"Task {task_id} not found for claim by {owner}")
            return f"Error: Task {task_id} not found"
        task = json.loads(path.read_text())
        task["owner"] = owner
        task["status"] = "in_progress"
        path.write_text(json.dumps(task, indent=2))
    main_logger.info(f"Task #{task_id} claimed by {owner}")
    ...
    def _loop(self, name: str, role: str, prompt: str):
        ...
        while True:
            ...
            for iteration in range(50):
                ...
                try:
                    subagent_logger.info(f"Teammate '{name}' sending LLM request (work iteration {iteration + 1})")
            ...
            subagent_logger.info(f"Teammate '{name}' entering idle phase ({polls} polls)")
...
def handle_shutdown_request(teammate: str) -> str:
    req_id = str(uuid.uuid4())[:8]
    main_logger.info(f"Sending shutdown request to '{teammate}' (req: {req_id})")
    with _tracker_lock:
...
def handle_plan_review(request_id: str, approve: bool, feedback: str = "") -> str:
    main_logger.info(f"Processing plan review for request {request_id}: {'approved' if approve else 'rejected'}")
    with _tracker_lock:
        req = plan_requests.get(request_id)
    if not req:
        main_logger.warning(f"Unknown plan request_id '{request_id}'")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            query = input(f"{Colors.CYAN}s11 >> {Colors.RESET}")
