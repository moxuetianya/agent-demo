...
setup_logging()
main_logger = get_logger("main_full")
...
class TodoManager:
    def __init__(self):
        self.items = []
        task_logger.info("TodoManager initialized")
    ...
    def update(self, items: list) -> str:
        ...
        task_logger.info(f"Updated {len(validated)} todos")
        return self.render()
...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    subagent_logger.info(f"Spawning subagent (type={agent_type}): {prompt[:100]}...")
    sub_tools = [
    ...
    resp = None
    for i in range(30):
        subagent_logger.info(f"Subagent iteration {i + 1}")
    ...
    if resp:
        result_text = "".join(b.text for b in resp.content if hasattr(b, "text")) or "(no summary)"
        subagent_logger.info(f"Subagent completed: {result_text[:100]}...")
        return result_text
    subagent_logger.warning("Subagent failed")
...
class SkillLoader:
    def __init__(self, skills_dir: Path):
        ...
        skill_logger.info(f"Loaded {len(self.skills)} skills from {skills_dir}")
    ...
    def load(self, name: str) -> str:
        ...
        if not s:
            skill_logger.warning(f"Unknown skill '{name}'")
            return f"Error: Unknown skill '{name}'. Available: {', '.join(self.skills.keys())}"
        skill_logger.info(f"Loading skill '{name}'")
...
def microcompact(messages: list):
    ...
    compact_logger.debug("Microcompact: cleared old tool results")
...
def auto_compact(messages: list) -> list:
    ...
    compact_logger.info(f"Auto-compact: transcript saved to {path}")
    ...
    compact_logger.info("Auto-compact completed")
    return [
...
class TaskManager:
    def __init__(self):
        ...
        task_logger.info(f"TaskManager initialized at {TASKS_DIR}")
    ...
    def create(self, subject: str, description: str = "") -> str:
        ...
        self._save(task)
        task_logger.info(f"Task created: #{task['id']} - {subject}")
    ...
    def update(self, tid: int, status: str = None,
        ...
        if status:
            ...
            if status == "deleted":
                ...
                task_logger.info(f"Task #{tid} deleted")
        ...
        self._save(task)
        task_logger.info(f"Task #{tid} updated: status={status}")
        return json.dumps(task, indent=2)
    ...
    def claim(self, tid: int, owner: str) -> str:
        ...
        self._save(task)
        task_logger.info(f"Task #{tid} claimed by {owner}")
...
class BackgroundManager:
    ...
    def run(self, command: str, timeout: int = 120) -> str:
        ...
        bg_logger.info(f"Starting background task {tid}: {command[:80]}")
        threading.Thread(target=self._exec, args=(tid, command, timeout), daemon=True).start()
        return f"Background task {tid} started: {command[:80]}"

    def _exec(self, tid: str, command: str, timeout: int):
        bg_logger.info(f"Task {tid} executing: {command[:80]}")
        try:
            r = subprocess.run(command, shell=True, cwd=WORKDIR,
                               capture_output=True, text=True, timeout=timeout)
            output = (r.stdout + r.stderr).strip()[:50000]
            self.tasks[tid].update({"status": "completed", "result": output or "(no output)"})
            bg_logger.info(f"Task {tid} completed with return code {r.returncode}")
        except Exception as e:
            self.tasks[tid].update({"status": "error", "result": str(e)})
            bg_logger.error(f"Task {tid} failed: {e}")
    ...
    def check(self, tid: str = None) -> str:
        ...
        bg_logger.info(f"Listing {len(self.tasks)} background tasks")
...
class MessageBus:
    ...
    def read_inbox(self, name: str) -> list:
        ...
        path.write_text("")
        if msgs:
            main_logger.info(f"Read {len(msgs)} messages from {name}'s inbox")
        return msgs
...
class TeammateManager:
    ...
    def _set_status(self, name: str, status: str):
        ...
        if member:
            ...
            self._save()
            subagent_logger.info(f"Teammate '{name}' status: {status}")

    def _loop(self, name: str, role: str, prompt: str):
        team_name = self.config["team_name"]
        subagent_logger.info(f"Teammate '{name}' ({role}) autonomous loop started")
            ...
            for iteration in range(50):
                ...
                for msg in inbox:
                    if msg.get("type") == "shutdown_request":
                        subagent_logger.info(f"Teammate '{name}' received shutdown request")
                ...
                if response.stop_reason != "tool_use":
                    subagent_logger.info(f"Teammate '{name}' completed work phase")
                    break
                results = []
                ...
                if idle_requested:
                    subagent_logger.info(f"Teammate '{name}' requested idle")
                    break
            # -- IDLE PHASE: poll for messages and unclaimed tasks --
            self._set_status(name, "idle")
            resume = False
            for poll in range(IDLE_TIMEOUT // max(POLL_INTERVAL, 1)):
                time.sleep(POLL_INTERVAL)
                inbox = self.bus.read_inbox(name)
                if inbox:
                    subagent_logger.info(f"Teammate '{name}' resuming due to inbox message")
                    for msg in inbox:
                        if msg.get("type") == "shutdown_request":
                            subagent_logger.info(f"Teammate '{name}' shutting down from idle")
                            self._set_status(name, "shutdown")
                            return
                        messages.append({"role": "user", "content": json.dumps(msg)})
                    resume = True
                ...
                if unclaimed:
                    ...
                    subagent_logger.info(f"Teammate '{name}' auto-claimed task #{task['id']}")
                    resume = True
                    break
            if not resume:
                subagent_logger.info(f"Teammate '{name}' idle timeout, shutting down")
                self._set_status(name, "shutdown")
                return
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        if estimate_tokens(messages) > TOKEN_THRESHOLD:
            compact_logger.info("Auto-compact triggered")
        ...
        # s06: manual compress
        if manual_compress:
            compact_logger.info("Manual compact triggered")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            query = input(f"{Colors.CYAN}s_full >> {Colors.RESET}")
        ...
        if query.strip() == "/compact":
            main_logger.info("Manual compact triggered via /compact")
            if history:
                print("[manual compact via /compact]")
                history[:] = auto_compact(history)
            continue
        if query.strip() == "/tasks":
            main_logger.info("Listing tasks")
