...
class EventBus:
    def __init__(self, event_log_path: Path):
        ...
        main_logger.info(f"EventBus initialized at {event_log_path}")
    ...
    def emit(
        ...
        main_logger.debug(f"Event emitted: {event}")
...
class TaskManager:
    ...
    def update(self, task_id: int, status: str = None, owner: str = None) -> str:
        ...
        self._save(task)
        task_logger.info(f"Task #{task_id} updated: status={status}, owner={owner}")
        return json.dumps(task, indent=2)
    ...
    def bind_worktree(self, task_id: int, worktree: str, owner: str = "") -> str:
        ...
        task["updated_at"] = time.time()
        self._save(task)
        task_logger.info(f"Task #{task_id} bound to worktree '{worktree}'")
        return json.dumps(task, indent=2)
    ...
    def unbind_worktree(self, task_id: int) -> str:
        ...
        task["updated_at"] = time.time()
        self._save(task)
        task_logger.info(f"Task #{task_id} unbound from worktree")
        return json.dumps(task, indent=2)
...
class WorktreeManager:
    def __init__(self, repo_root: Path, tasks: TaskManager, events: EventBus):
        ...
        main_logger.info(f"WorktreeManager initialized at {self.dir}")
    ...
    def _run_git(self, args: list[str]) -> str:
        ...
        tool_logger.info(f"Running git command: {' '.join(args)}")
        r = subprocess.run(
            ["git", *args],
            cwd=self.repo_root,
            capture_output=True,
        ...
        if r.returncode != 0:
            ...
            tool_logger.error(f"Git command failed: {msg}")
    ...
    def create(self, name: str, task_id: int = None, base_ref: str = "HEAD") -> str:
        ...
        main_logger.info(f"Creating worktree '{name}' (task_id={task_id}, base_ref={base_ref})")
        try:
            ...
            main_logger.info(f"Worktree '{name}' created successfully")
            ...
            main_logger.error(f"Failed to create worktree '{name}': {e}")
            raise

    def list_all(self) -> str:
        idx = self._load_index()
        wts = idx.get("worktrees", [])
        main_logger.info(f"Listing {len(wts)} worktrees")
        if not wts:
    ...
    def run(self, name: str, command: str) -> str:
        tool_logger.info(f"Running command in worktree '{name}': {command}")
        ...
        try:
            ...
            tool_logger.error("Command timed out after 300s")
    ...
    def remove(self, name: str, force: bool = False, complete_task: bool = False) -> str:
        ...
        main_logger.info(f"Removing worktree '{name}' (force={force}, complete_task={complete_task})")
        try:
            ...
            if complete_task and wt.get("task_id") is not None:
                ...
                main_logger.info(f"Task #{task_id} marked as completed")
            ...
            main_logger.info(f"Worktree '{name}' removed successfully")
            ...
            main_logger.error(f"Failed to remove worktree '{name}': {e}")
            raise
    ...
    def keep(self, name: str) -> str:
        ...
        main_logger.info(f"Worktree '{name}' marked as kept")
...
if __name__ == "__main__":
    ...
    if not WORKTREES.git_available:
        main_logger.warning("Not in a git repo. worktree_* tools will return errors.")
    ...
    while True:
        ...
        try:
            query = input(f"{Colors.CYAN}s12 >> {Colors.RESET}")
